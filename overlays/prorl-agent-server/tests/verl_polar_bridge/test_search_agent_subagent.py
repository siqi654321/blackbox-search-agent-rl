...
def test_adapter_can_disable_legacy_group_stitch(monkeypatch):
    monkeypatch.setenv("POLAR_STITCH_BY_MERGE_GROUP", "1")
    monkeypatch.setenv("POLAR_DISABLE_ADAPTER_LEGACY_STITCH", "1")
    ...
    assert [s.metadata["polar"].get("segment_kind") for s in samples] == [
        "final",
        "subagent",
        "subagent",
        "final",
    ]
    assert {s.metadata["polar"]["num_segments"] for s in samples} == {4}
    assert all("adapter_stitched_trace_count" not in s.metadata["polar"].get("trace_metadata", {}) for s in samples)
    assert {s.metadata["polar"].get("adapter_legacy_stitch_enabled") for s in samples} == {False}
    assert {s.metadata["polar"].get("adapter_legacy_stitch_disabled_reason") for s in samples} == {"env"}


def test_adapter_auto_disables_legacy_stitch_for_prompt_grounded_single_builder(monkeypatch):
    monkeypatch.setenv("POLAR_STITCH_BY_MERGE_GROUP", "1")
    monkeypatch.delenv("POLAR_DISABLE_ADAPTER_LEGACY_STITCH", raising=False)
    first = _trace(
        "sid:main",
        "final",
        [1, 2],
        ...
        prompt_grounded_single_merge_enabled=1,
    )
    second = _trace(
        "sid:main",
        "final",
    ...
    assert all("adapter_stitched_trace_count" not in s.metadata["polar"].get("trace_metadata", {}) for s in samples)
    assert {s.metadata["polar"].get("adapter_legacy_stitch_enabled") for s in samples} == {False}
    assert {s.metadata["polar"].get("adapter_legacy_stitch_disabled_reason") for s in samples} == {
        "builder_prompt_grounded_single"
    }
